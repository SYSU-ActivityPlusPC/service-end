...
func SignUpHandler(w http.ResponseWriter, r *http.Request) {
	...
	if isEmailExist := services.CheckIfEmailExist(jsonBody.Email); isEmailExist == true {
		w.WriteHeader(400)
		return
	}
...
func VerifyPCUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	if intVerify == 1 {
		...
		password = services.GeneratePassword(12)
		err = model.VerifyUser(intID, intVerify, user.Email, services.GetPassword(strconv.Itoa(user.ID), password), &now)
	} else {
	...
	go services.SendMail(msg.From, msg.To, msg.Content, msg.Subject)
